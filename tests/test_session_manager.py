...
class TestSessionManager:
    ...
    @patch('homey_assistant.agent.session.google.TTS')
    @patch('homey_assistant.agent.session.google.LLM')
    def test_create_session(self, mock_llm_class, mock_tts_class, mock_stt_class, mock_session_class):
    ...
    def test_create_session_without_vad(self, mock_session_class):
        ...
        with patch.object(SessionManager, '_configure_stt') as mock_stt, \
    ...
    @patch('homey_assistant.agent.session.noise_cancellation.BVC')
    @patch('homey_assistant.agent.session.RoomInputOptions')
    def test_create_room_input_options_error_handling(self, mock_room_options_class, mock_bvc):
    ...
    def test_session_manager_logging(self, mock_logger):
        ...
        mock_logger.info.assert_called_with("SessionManager initialized with configuration")
    ...
    def test_session_creation_logging(self, mock_logger):
        ...
        with patch('homey_assistant.agent.session.AgentSession') as mock_session_class, \
            ...
            mock_logger.info.assert_any_call("Creating agent session with 0 MCP servers")
