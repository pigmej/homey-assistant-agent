...
def _configure_agent_conversation_logger() -> None:
    ...
    conversation_formatter = logging.Formatter(
        "%(asctime)s [CONVERSATION] %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
    )
...
def log_performance(
    logger: logging.Logger, operation: str, duration_ms: float, **context: Any
) -> None:
    """
    ...
    log_with_context(
        logger, logging.INFO, f"Performance: {operation} completed", **context
    )
